...
from albuDataLoader import FaceImagesAlbu
from train_val_test_v2 import *
from gotcha_trn_val_tst import *
# import cv2
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------- #
# training the model
# milan - EffNetB4_FF -> used BCE loss as in the paper
# python train.py --model "efficientnetb4_ff" --dataset "milan_occ" --data-aug "milan" --loss "bce"  --save-model --save-log --tags "EffNetB4_FF_milan_occ_TL"
# python train.py --model "efficientnetb4_dfdc" --dataset "milan_no_occ" --data-aug "milan" --loss "bce"  --save-model --save-log --tags "EffNetB4_DFDC_milan_no_occ_TL"

# resume training from checkpoint
# python train.py --model "effnetb4" --dataset "gotcha_occ" --tl --num-epochs 15 --batch-size 64 --optimizer "adamw" --loss "bce" --tags "EffNetB4_gotcha_occ_TL" --resume “./results_TL/EffNetB4_gotcha_occ_TL/training/EfficientNet_B4_2024-10-02-12-03-54/best_checkpoint_2.pth”

# train milan models
# python train.py --model "effnetb4_ff" --dataset "gotcha_no_occ" --data-aug "milan"  --tl --num-epochs 15 --batch-size 64 --optimizer "adamw" --loss "bce" --tags "EffNetB4_FF_gotcha_no_occ_TL"
...
def get_args_parse():
    ...
    # parser.add_argument('--num-classes', type=int, default=1, help='Number of classes in the dataset')
    # parser.add_argument('--pretrained', type=bool, default=True, help='Use pre-trained weights for the model')
    parser.add_argument('--batch-size', type=int, default=32, help='Batch size for training')
    parser.add_argument('--num-epochs', type=int, default=10, help='Number of epochs for training')
    # TODO: try to increase the number of epochs -> 15, 20, 25, 30 (to see if the model improves)
    # -------------------------------------------------------- #
    ...
    # TODO: test lr = 2e-4, and weight decay = 5e-4 (for adam as done in deepfakebench)
    # milan guys used lr = 1e-5, and a lr scheduler (reduce on plateau) -> try to use it with focal loss
    # -------------------------------------------------------- #
    ...
    # old default: /home/boss/rz-test/bceWLL_test/model/best_model.pth
    # ./model/best_focal_loss_model/new_focal_loss_no_resize_rotation_color_jitter.pth
    ...
    # parser.add_argument('--resume-ckpt', action = 'store_true', help = 'resume training of last checkpoint')
    # add wandb argument
    ...
    # add gradcam argument
    # parser.add_argument('--gradcam', type=bool, default=False, help='Use gradcam for visualization')
...
def main(): 
    ...
    #         criterion = checkpoint['criterion']
    #         print(f"criterion {criterion} loaded from checkpoint!")
    # else:
    # Focal loss
    if args.loss == 'focal':
